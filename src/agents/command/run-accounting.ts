...
function normalizeAccountingTarget(error: unknown): Error {
  try {
  ...
  try {
    ...
    // Preserve accounting even when a hostile thrown value defeats formatting.
  }
  const fallback = new Error(message, { cause: error });
  try {
    const target = toErrorObject(error, message);
...
export async function runWithAgentCommandAccounting<T>(
  ...
  return await activeCommandRunAccounting.run(accounting, async () => {
    try {
      ...
      const target = normalizeAccountingTarget(error);
      bindAgentCommandRunAccounting(target, accounting.project());
      throw target;
    }
  });
}
...
export function createRunAccountingAccumulator(startedAtMs = Date.now()): RunAccountingAccumulator {
  ...
  return {
    ...
    project(): AgentCommandRunAccountingSnapshot {
      ...
      const agentSubmissionCoverageReasons = [...runtimeReasons];
