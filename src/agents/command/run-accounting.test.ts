...
describe("command run accounting", () => {
  it("accumulates embedded candidates, agent submissions, usage, tools, and lifecycle samples", () => {
    ...
    second.observeCodeModeFinalQuiescence("quiescent");
  ...
  it("attaches accounting to early Error and primitive failures", async () => {
    ...
    expect(caughtPrimitive).toBeInstanceOf(Error);
    expect((caughtPrimitive as Error).cause).toBe("provider exploded");
    expect(resolveAgentCommandRunAccounting(caughtPrimitive)?.coverage.candidates).toEqual({
      state: "unavailable",
      reasons: ["not_observed"],
    });
  });
