...
describe("CLI attempt execution", () => {
  ...
  it("forwards command accounting only to the embedded runner boundary", async () => {
    ...
    const commandRunAccounting = {
      ...
      observeCodeModeFinalQuiescence: vi.fn(),
      settle: vi.fn(),
    };
    ...
    expect(resolveEmbeddedRunAccountingObservers(embedded)).toEqual({
      ...
      onCodeModeFinalQuiescence: commandRunAccounting.observeCodeModeFinalQuiescence,
    });
    expect(resolveEmbeddedRunAccountingObservers(returnedResult ?? {})).toEqual(
      resolveEmbeddedRunAccountingObservers(embedded),
    );
  });
