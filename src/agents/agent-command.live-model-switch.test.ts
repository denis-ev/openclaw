...
describe("agentCommand – LiveSessionModelSwitchError retry", () => {
  ...
  it("samples Code Mode quiescence after command finalization", async () => {
    ...
    state.runAgentAttemptMock.mockImplementation(async (attemptParams: unknown) => {
      const attempt = attemptParams as {
        ...
        commandRunAccounting: {
          ...
          observeCodeModeFinalQuiescence: (
            state: "quiescent" | "non_quiescent" | "unavailable",
          ) => void;
        };
      };
      attempt.commandRunAccounting.selectRuntime("embedded");
      ...
      return bindEmbeddedRunAccountingObservers(
        makeSuccessResult(attempt.providerOverride, attempt.modelOverride),
        {
          codeModeActivityOwner: activityOwner,
          onCodeModeFinalQuiescence: attempt.commandRunAccounting.observeCodeModeFinalQuiescence,
        },
      );
    });
    state.deliverAgentCommandResultMock.mockImplementation(async (params: unknown) => {
      const result = requireRecord(requireRecord(params, "delivery params").result, "result");
      expect(resolveEmbeddedRunAccountingObservers(result)?.codeModeActivityOwner).toBe(
        activityOwner,
      );
  ...
  it("classifies empty embedded run results before model fallback accepts them", async () => {
    ...
    state.runAgentAttemptMock
      .mockImplementationOnce(async (attemptParams: unknown) => {
        ...
        return bindEmbeddedRunAccountingObservers(makeEmptyResult("anthropic", "claude"), {
          codeModeActivityOwner: attempt.commandRunAccounting.codeModeActivityOwner,
        });
      })
      .mockImplementationOnce(async (attemptParams: unknown) => {
        const attempt = attemptParams as {
        ...
        return bindEmbeddedRunAccountingObservers(makeSuccessResult("openai", "gpt-5.4"), {
          codeModeActivityOwner: winningOwner,
        });
      });
    ...
    expect(resolveEmbeddedRunAccountingObservers(result)?.codeModeActivityOwner).toBe(winningOwner);
