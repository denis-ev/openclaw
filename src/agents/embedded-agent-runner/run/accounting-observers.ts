...
type EmbeddedRunAccountingObservers = {
  ...
  onCodeModeFinalQuiescence?: (state: CodeModeRunFinalQuiescence) => void;
};
...
export function bindEmbeddedRunAccountingObservers<T extends object>(
  ...
  if (
    ...
    value?.onOpaqueWork ||
    value?.onCodeModeFinalQuiescence
  ) {
...
export function observeEmbeddedRunFinalQuiescence(target: object): void {
  const accountingObservers = observers.get(target);
  accountingObservers?.onCodeModeFinalQuiescence?.(
    sampleCodeModeRunFinalQuiescence(accountingObservers.codeModeActivityOwner),
  );
}
