...
const CODE_MODE_CATALOG_INDEX_HEADING = [
  ...
  "OUTPUT DECLARED RULE: use declared fields for deterministic transforms and dependent calls; `tools.search` catalog lookup stays inline. Mutate application state only with a stable target ID from request/trusted prior context; application target identity discovered from tool data returns candidates/evidence first.",
...
export function createCodeModeTools(ctx: CodeModeToolContext): AnyAgentTool[] {
  const execTool = markCodeModeControlTool({
    ...
    parameters: Type.Object({
      ...
      code: Type.String({
        description:
          "Run JavaScript or TypeScript in OpenClaw Code Mode. `return` sets result; else `null`. Declared fields enable deterministic transforms/dependent calls; `tools.search` resolves catalog IDs inline. Mutate app state only with a stable target ID from request/trusted prior context. Application target identity discovered from tool data returns candidates/evidence before later mutation. Return authority/permission/ownership conflicts as evidence. `-> ?`: return raw, inspect later. Use exact enabled catalog tools for shell/file/network/external actions; raw shell commands and Node `require`/`import` are unavailable.",
      }),
