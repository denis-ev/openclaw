...
describe("Code Mode catalog and model-visible surface", () => {
  ...
  it("describes deterministic OpenClaw Code Mode composition in the exec schema", () => {
    ...
    expect(execTool.description).toContain(
      "request or trusted prior context supplies a stable target ID",
    );
    expect(execTool.description).toContain(
      "Application target identity discovered from tool data returns candidates or evidence for model observation before a later mutation",
    );
    expect(execTool.description).toContain(
    ...
    expect(parameters.properties?.code?.description).toContain(
      "stable target ID from request/trusted prior context",
    );
    expect(parameters.properties?.code?.description).toContain(
      "Application target identity discovered from tool data returns candidates/evidence before later mutation",
    );
    expect(parameters.properties?.code?.description).toContain(
  ...
  it("primes the exec schema with exact native tool ids and compact contracts", () => {
    ...
    expect(description).toContain("stable target ID from request/trusted prior context");
