...
When tool data discovers application target identity, return candidates or
evidence for model observation instead of mutating in that cell:
...
The next model turn can choose the observed stable ID and perform the mutation.
This boundary applies to application data regardless of whether code found one
or several candidates; it does not impose an extra turn on catalog discovery.
Conflicting authority, permission, or ownership evidence also returns before
mutation. Normal tool policy and approvals still apply in every case.
