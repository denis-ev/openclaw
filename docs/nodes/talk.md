...
WebRTC calls through the Gateway and keep the authenticated sideband there.
Platform keys use `/v1/live`; ChatGPT OAuth uses the Codex backend call route.
Other GPT-Live consumers use the direct Frameless Bidi WebSocket and remain
...
Talk catalog. Browser and Gateway-relay Talk can use either an eligible Platform
key or the selected OpenClaw ChatGPT OAuth profile. GPT-Live also requires the
bundled `openai` plugin registered in full mode; a restrictive `plugins.allow`
list fails session creation with "OpenAI GPT-Live browser session broker is
unavailable". Runtime bounds: 8 concurrent sessions per Gateway and a 30-minute
session TTL. Browser sessions also use 60-second single-use offer tokens.
