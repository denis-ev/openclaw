...
- **Host policy is injected, not bundled.** Request fetch guarding (for
  ...
  evidence and does not rewrite the requested provider/model/API identity. The library
  default observer is inert; OpenClaw installs its collector in its stream
  facade. Provider coverage depends on which adapters emit these facts.
